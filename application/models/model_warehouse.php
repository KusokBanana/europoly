...
{
    var $products_warehouses_table = "products_warehouses
    ...
    function getDTProductsForAllWarehouses($input)
    {
        $product_warehouses_columns = array(
            array('dt' => 0, 'db' => 'products_warehouses.product_warehouse_id'),
            ...
            array('dt' => 7, 'db' => 'products.purchase_price'),
            array('dt' => 8, 'db' => 'products_warehouses.buy_and_taxes'),
            array('dt' => 9, 'db' => 'products_warehouses.sell_price'),
            array('dt' => 10, 'db' => 'products.dealer_price'),
            array('dt' => 11, 'db' => 'products_warehouses.total_price')
        );
        $this->sspSimple($this->products_warehouses_table, "products_warehouses.product_id", $product_warehouses_columns, $input);
    }
    ...
    function getDTProductsForWarehouse($warehouse_id, $input)
    {
        ...
        $this->sspComplex($this->products_warehouses_table, "products_warehouses.product_id", $product_warehouses_columns, $input, null, "products_warehouses.warehouse_id = $warehouse_id");
    }

    function addProductsWarehouse($product_id, $warehouse_id, $amount, $buy_price, $buy_and_taxes, $sell_price, $dealer_price, $total_price)
    {
    ...
    function getPrices($warehouse_id)
    {
        ...
        $where = ($warehouse_id) ? "WHERE warehouse_id = $warehouse_id" : "";
        $warehouseProducts = $this->getAssoc("SELECT * FROM products_warehouses $where");
        if (!empty($warehouseProducts))
            foreach ($warehouseProducts as $warehouseProduct) {
                ...
                $buyAndExpenses += $warehouseProduct['buy_and_taxes'];
                $dealerPrice += $product['dealer_price'];
//                $sellPrice += $product['purchase_price'];

                $buyPrice *= $amount;
                $buyAndExpenses *= $amount;
                $dealerPrice *= $amount;
            }
        return [
            'buy' => $buyPrice,
